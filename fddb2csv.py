...
df2 = pd.DataFrame(columns = ['Frame', 'x_center', 'y_center', 'w', 'h', 'theta', 's','FileName'])
    ...
    for line in open(name):
        ...
        for i in range(int(num_img_faces)):
            ...
            if(theta> 0 ):
                ...
                orientations = 0
            data3 = {'Frame':Frame[0] + '.jpg','x_center': np.round(center_x), 'y_center': np.round(center_y), 'w': np.round(ww), 'h': np.round(hh), 'theta': orientations, 's':s, 'FileName':img_loc }
