...
class graphical_model:
    def __init__(self, mc):
        ...
        self.num_gta = tf.placeholder(tf.int32, name = 'num_gta')
        self.gt_boxes = tf.placeholder(tf.float32, [None, 5], name = 'gt_boxes')#because now only support batch=1 as input
        self.target_anchor_box_labels = tf.placeholder(tf.float32, [mc.BATCH_SIZE, mc.H, mc.W, mc.ANCHOR_PER_GRID , mc.CLASSES], name = 'target_anchor_box_labels')
        self.target_cls_label = tf.placeholder(tf.float32, [mc.BATCH_SIZE, mc.H, mc.W, mc.ANCHOR_PER_GRID , 20], name = 'target_cls_label')
    ...
    def loss_func(self, sigma_rpn=3.0):
        ...
        with tf.variable_scope('cnn-loss') as scope:
            ...
            #self._anchor_targets['target_cls_label'] = tf.to_int32(self.target_cls_label)
            #rpn. fg/bg loss
            ...
            #end2end classification loss
            #self._anchor_targets['cls_label'] = tf.cast(self.target_anchor_box_labels, tf.float32)
            #cls_score = tf.reshape(self._predictions['cls_score_reshape'], [-1, mc.CLASSES])
            #cls_score = tf.nn.softmax(cls_score)
            #cls_score = tf.reshape(cls_score, [-1, mc.CLASSES])

            #cls_label = tf.reshape(self._anchor_targets['cls_label'], [-1, mc.CLASSES])
            #cls_score = tf.reshape(tf.gather(cls_score, rpn_keep), [-1, mc.CLASSES])
            #cls_label = tf.reshape(tf.gather(cls_label, rpn_keep), [-1, mc.CLASSES])
            #self._predictions['cls_score_reshape'] = cls_score
            #self._anchor_targets['cls_label'] = cls_label
            #cls_cross_entropy_n = tf.nn.softmax_cross_entropy_with_logits(logits = cls_score, labels = cls_label)
            #cls_cross_entropy_n = cls_label * (-tf.log(cls_score+mc.EPSILON)) + (1 - cls_label) * (-tf.log(1 - cls_score + mc.EPSILON))
            #cls_cross_entropy = tf.reduce_mean(cls_cross_entropy_n)
            #cls_cross_entropy = tf.reduce_mean(cls_cross_entropy_n)
            #self._losses['cls_cross_entropy'] = cls_cross_entropy
            #correct_pred = tf.equal(tf.argmax(cls_score, 1), tf.argmax(cls_label, 1))
            #self.accuracy = tf.reduce_mean(tf.cast(correct_pred, tf.float32), name='accuracy')
            #cls regression
            # RCNN, class loss
            #cls_score = tf.reshape(self._predictions["cls_score"], [-1,2])
            #label = tf.reshape(self._anchor_targets["labels"], [-1])
            #cross_entropy = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=cls_score, labels=label))

            # RCNN, bbox loss
            #bbox_pred = self._predictions['bbox_pred']
            #bbox_targets = self._anchor_targets['bbox_targets']
            #bbox_inside_weights = self._anchor_targets['bbox_inside_weights']
            #bbox_outside_weights = self._anchor_targets['bbox_outside_weights']
            #loss_box = self._smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights)
            #cls_score = tf.reshape(self._predictions['cls_score_reshape'],[-1, 2])
            #cls_label = tf.reshape(self._anchor_targets['target_cls_label'], [-1])
            #fg_keep = tf.equal(cls_label, 1)
            ...
            #self._losses['cls_cross_entropy'] = cls_cross_entropy
            #self._losses['cls_cross_entropy'] = cls_cross_entropy
            loss = rpn_cross_entropy + rpn_loss_box #+ cls_cross_entropy#+ cross_entropy + loss_box#+ cls_cross_entropy
