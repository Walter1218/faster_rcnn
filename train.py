...
def train():
    with tf.Graph().as_default():
        ...
        for step in xrange(FLAGS.max_steps):
            ...
            #else:
            labels, bbox_targets, bbox_inside_weights, bbox_outside_weights , groundtruth, target_anchor_box_labels = batch_generate.target_label_generate(bb_boxes, anchor_box, mc, DEBUG = False)
            num_gta = len(groundtruth)
            #print(num_gta)
            feed_dict = {
                         ...
                         model.target_anchor_box_labels: np.expand_dims(target_anchor_box_labels, axis = 0),
                         #model.target_cls_label: np.expand_dims(target_cls_label, axis = 0),
                         #model.cls_map: np.expand_dims(cls_map, axis = 0),# for end2end classification
            }#, model._predictions['cls_score_reshape'], model._anchor_targets['cls_label'], model.accuracy
            losses = sess.run([model._losses, model.train_op, model.det_proposaled_scores], feed_dict = feed_dict)
            print('the training step is {0}, number of groundtruth is {1}, and losses is {2}, cls accuracy is {3}'.format(step, num_gta, losses[0:2], losses[-1]))
            #print(losses[-1].shape, losses[-2].shape)
